...
namespace trimesh {
    ...
    vec3d triangle_angles( vec3d A, vec3d B, vec3d C ){
        double a_ = (B-C).length();
        double b = (A-C).length();
        double c_ = (A-B).length();
        double cosalpha = std::max( -1.0, std::min( 1.0, (b*b+c_*c_-a_*a_)/(2.0*b*c_) ) );
        double cosbeta  = std::max( -1.0, std::min( 1.0, (a_*a_+c_*c_-b*b)/(2.0*a_*c_) ) );
        double cosgamma = std::max( -1.0, std::min( 1.0, (a_*a_+b*b-c_*c_)/(2.0*a_*b) ) );
        vec3d ret( acos(cosalpha),acos(cosbeta),acos(cosgamma));
        if( fabs(ret[0]+ret[1]+ret[2]-M_PI) > 1e-5 ){
            std::cout << a_ << ", " << b << ", " << c_ << ", " << cosalpha << ", " << cosbeta << ", " << cosgamma << ", " << ret[0]+ret[1]+ret[2]-M_PI << std::endl;
